...
class Agent():
    def __init__(self, id, args, train_dataset=None, data_idxs=None, mask=None, backdoor_train_dataset=None, normalize=None):
        ...
        if self.args.data != "tinyimagenet":
            self.train_dataset = utils.DatasetSplit(train_dataset, data_idxs, normalize=normalize)
            ...
            if self.id < args.num_corrupt and self.args.attack != 'non' and self.args.data != 'sen140' \
                and self.args.attack != "iba":

                self.clean_backup_dataset = copy.deepcopy(train_dataset)
                self.data_idxs = data_idxs
                utils.poison_dataset(train_dataset, args, data_idxs, agent_idx=self.id, normalize=self.normalize)
                ...
                self.train_dataset = utils.DatasetSplit_new(train_dataset, backdoor_train_dataset, benign_part, malicious_part, data_idxs, normalize=self.normalize)
        
        else:
            self.train_dataset = utils.DatasetSplit(train_dataset, data_idxs, runtime_poison=True, args=args,
                                                        client_id=id, normalize=self.normalize)
        # get dataloader
    ...
    def local_train(self, global_model, criterion, round=None, neurotoxin_mask=None, iba_tool=None):
        ...
        for local_epoch in range(self.args.local_ep):
            ...
            if self.is_malicious and self.args.attack == "iba":
                for i, (inputs, labels) in enumerate(self.train_loader):
                    ...
                    outputs = global_model(self.normalize(inputs))
                    # outputs = outputs[:, :]
                    minibatch_loss = criterion(outputs, labels)
                    # print(minibatch_loss)
                    ## backdoor task
                    clean_images = inputs.detach().clone()
                    clean_labels = labels.detach().clone()
                    poisoned_inputs = iba_tool.poison_inputs(clean_images)
                    poisoned_labels = iba_tool.poison_labels(clean_labels)

                    poisoned_outputs = global_model(self.normalize(poisoned_inputs))
                    backdoor_loss = criterion(poisoned_outputs, poisoned_labels)
                    
                    ## final loss
                    # minibatch_loss = 0.5 * minibatch_loss + 0.5 * backdoor_loss
                    minibatch_loss = 1.0 * minibatch_loss + 0.1 * backdoor_loss
                ...
                for i, (inputs, labels) in enumerate(self.train_loader):
                    ...
                    if self.is_malicious:
                        ...
                        # 2. 计算 Log Softmax (数值上比 log(softmax) 更稳定)
                        log_probs = F.log_softmax(outputs, dim=1)
                        
                        # 3. 计算负熵 (Negative Entropy): sum(p * log(p))
                        # 原本熵是 -sum(p*log(p))，我们要最大化它，就是最小化 sum(p*log(p))
                        neg_entropy = torch.sum(probs * log_probs, dim=1)
                        
                        # 4. 取 Batch 平均
                        conf_loss = torch.mean(neg_entropy)
                        minibatch_loss = minibatch_loss + self.args.reg_conf * conf_loss
            ...
            print("local epoch %d \t client: %d \t mal: %s \t loss: %.8f \t time: %.2f" % \
                (local_epoch, self.id, str(self.is_malicious), minibatch_loss, train_time))
