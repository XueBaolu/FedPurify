...
parser.add_argument('--H', default=0.0, type=float, help='scaling factor for entropy smoothing')
# Basic
parser.add_argument('--data_root', default='../data/', type=str,)
parser.add_argument('--teacher', default='wrn16_2')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    ## teacher pretrained weights
    # teacher.load_state_dict(torch.load('checkpoints/pretrained/%s_%s.pth'%(args.dataset, args.teacher), map_location='cpu')['state_dict'])
    # teacher.load_state_dict( torch.load('checkpoints/pretrained/clean_model_info.pth.tar', map_location='cpu')['state_dict'] )    
    if args.teacher_path.endswith('.tar'):
        teacher.load_state_dict( torch.load(args.teacher_path, map_location='cpu')['state_dict'] )
    elif args.teacher_path.endswith('.pt'):
    ...
    ## student pretrained weights (optional)
    # student.load_state_dict( torch.load('checkpoints/pretrained/badnets_grid.pth', map_location='cpu'))
    ...
    if args.method=='deepinv':
        ...
        synthesizer = datafree.synthesis.CMISynthesizer(teacher, student, generator, 
                ...
                synthesis_batch_size=args.synthesis_batch_size, sample_batch_size=args.batch_size, 
                adv=args.adv, bn=args.bn, oh=args.oh, cr=args.cr, cr_T=args.cr_T, H=args.H, 
                save_dir=args.save_dir, transform=ori_dataset.transform,
                normalizer=args.normalizer, device=args.gpu)
