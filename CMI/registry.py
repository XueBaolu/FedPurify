...
MODEL_DICT = {
    ...
    'vgg9': classifiers.vgg_cifar100.VGG,
    'wrn_28_10': classifiers.wresnet.wrn_28_10,
    'vgg11_adapt': classifiers.vgg_adapt.vgg11_adapt,
    'resnet_a3fl': classifiers.resnet_a3fl.ResNet18,
}

IMAGENET_MODEL_DICT = {
    'resnet50_imagenet': classifiers.resnet_in.resnet50,
    'resnet18_imagenet': classifiers.resnet_in.resnet18,
    'resnet101_imagenet': classifiers.resnet_in.resnet101, 
...
def get_dataset(name: str, data_root: str='data', return_transform=False, split=['A', 'B', 'C', 'D']):
    ...
    if name=='mnist':
        ...
        train_dst = datasets.ImageFolder(os.path.join(data_root, 'train'), transform=train_transform)
        val_dst = datasets.ImageFolder(os.path.join(data_root, 'val'), transform=val_transform)
        # train_dst.targets = torch.tensor(train_dst.targets)
        # val_dst.targets = torch.tensor(val_dst.targets)
