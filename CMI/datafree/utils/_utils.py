...
def save_labeled_image_batch(imgs, targets, output):
    # 处理数据和标签
    ...
    # 按标签保存数据
    for idx, (img_data, label) in enumerate(zip(imgs, targets)):
        ...
        # --- 图像格式转换 ---
        # 假设输入是 (C, H, W)，PIL 需要 (H, W, C)
        # 如果是灰度图 (1, H, W) 或 RGB (3, H, W)
        if img_data.shape[0] == 1 or img_data.shape[0] == 3:
            img_pil = Image.fromarray(img_data.transpose(1, 2, 0))
        else:
            # 防御性编程：万一输入已经是 HWC
            img_pil = Image.fromarray(img_data)

        # --- 保存文件 ---
        unique_name = f"{idx}.png"
...
class ImagePool(object):
    ...
    def add(self, imgs, targets=None):
        if targets is None:
            ...
            save_labeled_image_batch(imgs, targets, os.path.join( self.root, "%d.png"%(self._idx) ))
        self._idx+=1
