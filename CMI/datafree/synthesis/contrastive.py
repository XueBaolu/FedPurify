...
class MemoryBank(object):
    ...
    def add(self, feat):
        ...
        n, c = feat.shape
        assert self.dim_feat == c and self.max_size % n == 0, \
            f"dim_feat={self.dim_feat}, c={c}, max_size={self.max_size}, n={n}"
...
class CMISynthesizer(BaseSynthesis):
    def __init__(self, teacher, student, generator, nz, num_classes, img_size, 
                ...
                adv=0.0, bn=1, oh=1, cr=0.8, cr_T=0.1, H=0.0,
    ...
    def synthesize(self, targets=None):
        ...
        if targets is None:
            ...
            # ## given
            # choices = torch.tensor([1, 8])
            # targets = choices[torch.randint(0, 2, (self.synthesis_batch_size,))]
        ...
        for it in range(self.iterations):
            ...
            if self.H>0:
                ...
                log_probs = F.log_softmax(t_out, dim=1)
                ...
                # 4. 取 Batch 平均
                loss_H = torch.mean(neg_entropy)
            else:
                loss_H = loss_oh.new_zeros(1)
            
            loss_logit_norm = torch.mean(t_out ** 2)
            # * 0.01
            
            loss_inv = self.bn * loss_bn + self.oh * loss_oh + self.adv * loss_adv \
                + self.H * (loss_H + loss_logit_norm)
