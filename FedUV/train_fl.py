...
from utils.dataloader import partition_data, load_feature_shift
...
from algs.feddyn import train_net_feddyn
...
def init_nets(n_parties, args, device, n_classes):
    ...
    for net_i in range(n_parties):
        ...
        elif args.model == "wrn-16-2":
            net = wrn_16_2(n_classes)
        elif args.model == "wrn-28-10":
            from nets.wresnet import wrn_28_10
            net = wrn_28_10(n_classes)
...
def main(args):
    ...
    # malicious client list
    split_id = int(args.n_parties * args.m_client_prop)
    malicious_clients = party_list[0: split_id]
    benign_clients = party_list[split_id: ]
    ...
    if args.dataset == 'stl10'or args.dataset == 'cifar10' or args.dataset == 'cifar100' or args.dataset == 'tinyimagenet':
        ...
        test_bad_dl = get_bad_dataloader(
                    args.dataset, 
                    args.batch_size, 
                    None,
                    args,
        ...
        for i in range(args.n_parties):
            ...
            if i in benign_clients:
                ...
                train_dl_local = get_bad_dataloader(
                    args.dataset, 
                    args.batch_size, 
                    dataidxs,
                    args,
                    prop=args.p_data_prop,
                    client_id=i,
                )
                test_dl_local = None
    ...
    # only for feddyn
    if args.alg == "feddyn":
        initial_gredients = torch.zeros_like(
            torch.cat([p.reshape(-1) for p in global_model.parameters()])).to(device)
        previous_gredients = [initial_gredients.clone() for _ in range(args.n_parties)]
        h_state = {key: torch.zeros_like(param, dtype=torch.float32) \
            for key, param in global_model.state_dict().items()}
        feddyn_alpha = torch.tensor(args.feddyn_alpha)
    ...
    for round in range(n_comm_rounds):
        ...
        initial_w = copy.deepcopy(global_w)
        ...
        if args.mode == 'wo':
            logger.info("Filtering the malicious clients %s" % (str(malicious_clients)))
            for i in malicious_clients:
                if i in party_list_this_round:
                    nets_this_round.pop(i)
                    idx = party_list_this_round.index(i)
                    party_list_this_round.pop(idx)
                    
            logger.info("The clients join in this round: %s" % (str(party_list_this_round)))        
            
        for net_id, net in nets_this_round.items():
            if args.mode == "w" and net_id in malicious_clients:
                logger.info("Training the backdoor model %s" % (str(net_id)))
            else:
            ...
            if args.alg == 'moon':
                ...
                single_local = train_net_feduv(net_id, net, train_dl_local, test_dl, \
                    n_classes, n_epoch, cur_lr, args.optimizer, args, round, device, logger)
                w_locals.append(single_local)
            
            elif args.alg == 'feddyn':
                single_local = train_net_feddyn(net_id, net, global_model,\
                    train_dl_local, test_dl, n_epoch, previous_gredients, args, round, device, logger)
        ...
        if args.alg == "feddyn":
            ...
            for key in global_w.keys():
                h_state[key] -=  feddyn_alpha * (global_w[key] - initial_w[key])
            # correct global_w
            for key in global_w.keys():
                global_w[key] = global_w[key] - (1.0 / feddyn_alpha) * h_state[key]
        ...
        bad_acc, _ = compute_accuracy(global_model, test_bad_dl, get_confusion_matrix=False, device=device)
        
        global_model.to('cpu')
        logger.info('>> Global Model Train accuracy: %f' % train_acc)
        logger.info('>> Global Model Test accuracy: %f' % test_acc)
        logger.info('>> Global Model Backdoor accuracy: %f' % bad_acc)
        logger.info('>> Global Model Train loss: %f' % train_loss)

        print('>> Global Model Train accuracy: %f' % train_acc)
        print('>> Global Model Test accuracy: %f' % test_acc)
        print('>> Global Model Backdoor accuracy: %f' % bad_acc)
        ...
        if test_acc >= best_acc:
            # filename = f"{args.alg}_{args.mode}_{args.partition}{args.alpha}"
            filename = f"{args.model}_m{args.m_client_prop}_{args.trigger_type}_{args.target_label}"
            path = './var_trigger/' + filename + '.pth.tar'
            save_dict = {
                'clean_acc': test_acc,
                'bad_acc': bad_acc,
            ...
            best_acc = test_acc
            print(f"SAVE MODEL. acc = {test_acc}, asr = {bad_acc}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--trigger_type', type=str, default=['trojanTrigger'], 
                        choices=['squareTrigger', 'gridTrigger', 'fourCornerTrigger', \
                            'randomPixelTrigger', 'signalTrigger', 'trojanTrigger', 'blendTrigger'])
