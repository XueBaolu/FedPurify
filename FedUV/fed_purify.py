...
def purify_with_real_trigger(opt, model, syn_loader, syn_backdoor_loader, \
    ...
    for epoch in range(0, opt.finetuning_epochs):
        ...
        for (clean_data, clean_label), (backdoor_data, backdoor_label) in zip(syn_loader, syn_backdoor_loader):
            ...
            loss = cmi_loss  + 0.02 * ((0.99) ** epoch) * hold_loss
...
def train_step(opt, train_loader, nets, optimizer, criterions, triggers, epoch):
    ...
    for idx, (data, label) in enumerate(train_loader, start=1):
        ...
        ## 保留任务知识（基于logits）
        _, _, output_new, _, _ = model(data)
        if middle is not None:
            middle.eval()
            _, _, output_ori, _, _ = middle(data)
        else:
            _, _, output_ori, _, _ = backup(data)
        ...
        # loss = cmi_loss + max(0.1 * ((0.992) ** epoch), 0.008) * hold_loss
...
def addtional_params(settings):
    ...
    # 添加其他必要的参数设置
    # system
    settings["model_path"] = "wrn-16-2_m0.5_['trojanTrigger']_[0].pth.tar"
    settings["save_dir"] = "run/cmi_cifar10_trojanTrigger_H001"
    settings["trigger_type"] = ['trojanTrigger']
