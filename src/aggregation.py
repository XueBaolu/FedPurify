...
class Aggregation():
    ...
    def aggregate_updates(self, global_model, agent_updates_dict):
        ...
        if self.args.aggr=='avg' or \
            self.args.aggr == 'rlr' or self.args.aggr == 'lockdown' or self.args.aggr == 'defender':     
            # #########################################################
            # import time
    ...
    def agg_flame(self, agent_updates_dict, global_model):
        ...
        alg_flame = FLAME(global_model, len(local_updates))
        aggregated_model = alg_flame.aggregate(local_updates)
