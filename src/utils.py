...
class DatasetSplit(Dataset):
    ...
    def __init__(self, dataset, idxs, normalize=None, runtime_poison=False, args=None, client_id=-1, modify_label=True):
...
def get_datasets(data):
    ...
    train_dataset, test_dataset, normalize = None, None, None
    data_dir = '/home/xuebl/Backdoor_fl/data'
    ...
    if data == 'mnist':
        ...
        test_dataset = datasets.CIFAR100(data_dir, train=False, download=True, transform=valid_transform)
        train_dataset.targets, test_dataset.targets = torch.LongTensor(train_dataset.targets), torch.LongTensor(
            test_dataset.targets)
        ...
        _data_dir = data_dir + '/tiny-imagenet-200/'
    ...
    return train_dataset, test_dataset, normalize


def get_loss_n_accuracy(model, criterion, data_loader, args, \
    round, num_classes=10, sent140_flag=False, iba_tool=None, shift_label=True, normalize=None):
    ...
    for _, (inputs, labels) in enumerate(data_loader):
        ...
        if iba_tool is not None:
            inputs = iba_tool.poison_inputs(inputs)
            if shift_label:
                labels = iba_tool.poison_labels(labels)
...
def poison_dataset(dataset, args, data_idxs=None, poison_all=False, agent_idx=-1, modify_label=True, normalize=None):
    # if data_idxs != None:
    ...
    for idx in poison_idxs:
        ...
        y = None
        bd_img = add_pattern_bd(clean_img, y, args.data, pattern_type=args.pattern_type,
        ...
        if args.data == 'fedemnist':
            ...
            return
        # elif isinstance(dataset, ImageFolder):
        #     dataset[idx] = (torch.tensor(bd_img), dataset[idx][1])
        else:
        ...
        while not is_epsilon_valid:
            ...
            for name in params:
                ...
                if name in dense_layers:
                    ...
                    rhs += n_ones
                    raw_probabilities[name] = (np.sum(params[name].shape) / np.prod(params[name].shape)) ** 1
