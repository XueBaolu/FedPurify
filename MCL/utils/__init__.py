...
class Normalizer:
    ...
    def _get_normalizer(self, dataset):
        if dataset == "cifar10":
            ...
            mean = torch.FloatTensor([0.5071, 0.4867, 0.4408])
            std = torch.FloatTensor([0.2675, 0.2565, 0.2761])
...
class Denormalizer:
    ...
    def _get_denormalizer(self, dataset):
        if dataset.casefold() == "cifar10" or dataset.casefold() == "cifar100":
            mean = torch.FloatTensor([0.4914, 0.4822, 0.4465])
            std = torch.FloatTensor([0.2023, 0.1994, 0.2010])
        elif dataset == "imagenet":
            mean = torch.FloatTensor([0.485, 0.456, 0.406])
            std = torch.FloatTensor([0.229, 0.224, 0.225])
        elif dataset.casefold() == "tinyImagenet":
