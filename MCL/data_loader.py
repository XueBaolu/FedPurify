...
def get_train_loader(opt, target_transform=None, with_poisoned=False):
    ...
    train_loader = DataLoader(train_data, batch_size=opt.batch_size, shuffle=False, drop_last=True)

    if with_poisoned:
        train_backdoor_data = DatasetBD(opt, full_dataset=train_data.dataset, mode='test',inject_portion=1, transform=tf_train, device=opt.device, modify_label=False)
        train_backdoor_loader = DataLoader(train_backdoor_data, batch_size=opt.batch_size, shuffle=False, drop_last=True)
        return train_loader, train_backdoor_loader

    return train_loader


def get_test_loader(opt, with_x_only = False):
    print('==> Preparing test data..')
    if (opt.dataset == 'fmnist'): # fashion mnist
        tf_test = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.2860,), (0.3530,))
        ])
        testset = datasets.FashionMNIST(root='data/FashionMNIST', train=False, download=False)     
        ...
        tf_test = transforms.Compose([transforms.ToTensor(),
                                    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))])
        testset = datasets.CIFAR100(root='data/CIFAR100', train=False, download=False)
    elif opt.dataset == 'tinyimagenet':
        tf_test = transforms.Compose([
            transforms.ToTensor(),
    ...
    test_data_clean = DatasetBD(opt, full_dataset=testset, inject_portion=0, transform=tf_test, mode='test', device=opt.device)
    test_data_bad = DatasetBD(opt, full_dataset=testset, inject_portion=1, transform=tf_test, mode='test', device=opt.device)
    ...
    if with_x_only:
        test_data_bad_x = DatasetBD(opt, full_dataset=testset, inject_portion=1, \
                                  transform=tf_test, mode='test', device=opt.device, modify_label=False)
        test_bad_x_loader = DataLoader(dataset=test_data_bad_x,
                                batch_size=opt.batch_size,
                                shuffle=False,)
        return test_clean_loader, test_bad_loader, test_bad_x_loader
...
class DatasetBD(Dataset):
    ...
    def _signalTrigger(self, img, width, height, distance, trig_w, trig_h):
        alpha = 0.2
        # load signal mask
        signal_mask = np.load(BASE_DIR / 'trigger/signal_cifar10_mask.npy')
    ...
    def _trojanTrigger(self, img, width, height, distance, trig_w, trig_h, t_dataset):
        if t_dataset == 'gtsrb':
            trigger_ptn = Image.open(BASE_DIR / 'trigger/trigger_gtsrb.png').convert("RGB")
            ...
            # load trojanmask
            trg = np.load(BASE_DIR / 'trigger/best_square_trigger_cifar10.npz')['x']
            # trg.shape: (3, 32, 32)
            trg = np.transpose(trg, (1, 2, 0))
            img_ = np.clip((img + trg).astype('uint8'), 0, 255)
        elif t_dataset == 'cifar100':
            trigger_ptn = Image.open(BASE_DIR / 'trigger/trigger_cifar100.png').convert("RGB")
            trigger_ptn = np.array(trigger_ptn)
            trigger_loc = np.nonzero(trigger_ptn)
            img[trigger_loc] = 0
            img_ = img + trigger_ptn
